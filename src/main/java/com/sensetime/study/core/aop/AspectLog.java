package com.sensetime.study.core.aop;
...
import com.sensetime.study.core.systemlog.SystemLogQueue;
...
public class AspectLog {
	 ...
	 * 定义切点
	 */
	@Pointcut("@annotation(com.sensetime.study.core.aop.AnnotationLog)")
	...
	private SystemLog getSystemLogInit(JoinPoint p) {
		...
		try {
			...
			Map<String, Object> nameAndArgs = getFieldsName(this.getClass(),
					clazzName, tartgetMethod, p.getArgs());
		...
		for (Method m : method) {
			if (m.getName().equals(methodName)) {
				...
				if (tmpCs.length == arguments.length) {
					AnnotationLog methodCache = m
							.getAnnotation(AnnotationLog.class);
					if (methodCache != null) {
